...
`, attachment: fs.createReadStream(__dirname + "/Nayan/salam.mp4")} ,threadID));
		...
		try {
			...
			const randomPath = readdirSync(join(__dirname, "cache", "salam.mp4", "randomgif"));

			if (existsSync(pathGif)) formPush = { body: msg, attachment: createReadStream(pathGif), mentions }
			else if (randomPath.length != 0) {
				const pathRandom = join(__dirname, "cache", "salam.mp4", "randomgif", `${randomPath[Math.floor(Math.random() * randomPath.length)]}`);
